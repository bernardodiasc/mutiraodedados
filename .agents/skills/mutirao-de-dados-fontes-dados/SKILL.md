---
name: mutirao-de-dados-fontes-dados
description: Diretrizes de criação, manutenção e monitoramento de fontes de dados no ecossistema do Mutirão de Dados. Carregar ao criar novas tabelas cache, jobs de importação, regras de QA ou controles de limpeza de dados.
---
...
### 4. Sinais (qualidade, lacunas e investigativos)

- Toda fonte gera **sinais** nos três tipos aplicáveis — taxonomia normativa em [`docs/qualidade-dados.md`](/docs/qualidade-dados.md); detalhes operacionais na skill `mutirao-de-dados-sinais`.
- Alertas de qualidade: regras em `src/lib/data/qa.ts` (fonte simples) ou `src/lib/data/<fonte>/qualidade.ts`; lacunas em `<fonte>/lacunas.ts` (pós-importação); cruzamentos em `<fonte>/investigativos.ts`. Persistência sempre via `flagQA` em `qa_findings` (coluna `tipo`).
