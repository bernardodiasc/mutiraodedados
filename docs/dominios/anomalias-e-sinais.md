...
| Sinal investigativo | `/anomalias` (sinais derivados de contratos, em memória) e cards nas fichas de entidade (candidato, parlamentar, fornecedor) — sempre com o aviso de sinais. Os investigativos **persistidos** (cruzamentos TSE, `licitacao_sem_desfecho`) aparecem em `/qualidade` com selo de tipo + aviso de sinais | `/admin/sinais` mostra APENAS os sinais em memória de contratos; os persistidos são triados em `/admin/qualidade` com o filtro por tipo `investigativo` |
