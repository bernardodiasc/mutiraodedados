...
Chave `\<tipo>#<ano>#<UF>` (ex.: `receitas#2022#SP`), com `linhas_processadas`, `importados`, `completa`. Bens/receitas/despesas retomam pulando linhas já processadas (re-stream do zip sem re-upsert); **resultados e candidatos recomeçam** e são processados por arquivo inteiro numa rodada (ambos limitados por UF), pois precisam agregar/deduplicar antes do upsert; upsert por PK substitui. Em bens, `bens_total_declarado` dos candidatos tocados na rodada é recalculado a partir de `tse_bens_candidato_cache` (soma dos valores positivos), então o total fica certo mesmo quando os bens de um candidato atravessam o corte entre rodadas. Limpeza seletiva por entidade zera as chaves `\<tipo>#%` (ver `clearImportData`).
...
## Importação sob demanda e CPU

Pela ferramenta (`bun run importar tse_arquivo …`, detalhes em [automacao.md](../automacao.md#importação-sob-demanda-modo-nomeado)), a rodada é a mesma do painel: `sincronizarArquivoTse` com `userId` nulo e a `origem` (gatilho `ferramenta` e `execucao_id`). A linha no Histórico grava `resultado`, o gatilho, a execução e `mes` 1 como âncora da eleição. O schema da janela (`arquivoTseSchema` em `ingest.functions.ts`) recusa ano sem eleição, ano fora da janela (`dentroDaJanelaAnual`) e combinação que o TSE não publica; o painel valida o ano com o mesmo `anoEleicaoTseSchema`. Falha ao ler o arquivo não lança: grava a linha com o erro (classificado em `erro_origem` ou `erro_nosso`), não avança a varredura e termina a rodada. A ponte (`tse_ponte`) devolve o `offset` do lote seguinte no `cursor`; lacunas, sinais e doador↔fornecedor rodam por eleição, com os runners de `sinais.server.ts` restritos ao ano.

**CPU.** Espera de rede não conta como CPU no Worker; o que conta é descomprimir e ler o CSV. A retomada de bens, receitas e despesas reabre o zip e relê o arquivo **desde o byte 0**, pulando as linhas já feitas com o mesmo leitor caractere a caractere, então o custo de cada rodada cresce com o cursor. Medido em V8 (Node, CSV sintético no formato do TSE, 200 MB, linhas de ~760 bytes): inflar custa ~0,8 ms/MB e inflar + ler ~7,8 ms/MB (~6 µs por linha). Uma UF grande de contas — da ordem de 1 milhão de linhas, ~800 MB descomprimidos — faz ~60 mil linhas por rodada de 150 s (o gargalo é o upsert, não a CPU), ou ~17 rodadas; a última pula ~750 MB: ~6 s de CPU a esse ritmo, ~12 s com o Worker duas vezes mais lento, mais ~1–2 s das linhas da própria rodada. Cabe nos 30 s padrão, mas sem folga se o arquivo for o dobro do estimado. Por isso o `wrangler.jsonc` declara `limits.cpu_ms` = 300.000 (o máximo): mesmo a maior entrada que um zip sem zip64 comporta (4 GB descomprimidos) fica em ~60 s no último pulo, com folga. Candidatos e resultados leem o arquivo inteiro numa rodada — algumas centenas de MB no pior caso, poucos segundos. Evitar a releitura desde o byte 0 exigiria guardar o estado do inflate entre rodadas (um inflate próprio em vez do `DecompressionStream`); fica para quando o limite de 5 min não bastar.
