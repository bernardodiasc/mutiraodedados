...
3. **Parse** — números brasileiros (com vírgula) e datas DD/MM/AAAA são normalizados; números grandes preservam precisão (ver `portal-client.ts`).
...
5. **Auto-correção (Portal CGU)** — quando um valor da listagem é suspeito (< R$ 100), o ingest consulta o endpoint de detalhe. Se diferir em mais de 5%, corrige e registra um QA finding de auto-correção. Se a tentativa de detalhe falhar, o registro é **pulado** (preferimos não importar do que importar errado).
...
- **Portal CGU e Transferegov** usam o cliente compartilhado em `src/lib/data/real/portal-client.ts` (mesma autenticação, parser e heurística de detalhe).
...
- Portal CGU: ~8 req/s, com pausa de 125ms quando há consulta ao endpoint de detalhe.
