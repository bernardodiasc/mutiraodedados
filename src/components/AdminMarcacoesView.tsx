...
import {
  ...
  severidadeFromVotos,
  STATUS_CONTESTACAO,
  TIPO_CONTESTACAO_LABEL,
  type Aba,
  type Contestacao,
  type StatusContestacao,
} from "@/lib/admin-marcacoes/logic";
import type { agregadoMarcacoes, listarMarcacoesAdmin } from "@/lib/data/marcacoes.functions";

type Marcacao = Awaited<ReturnType<typeof listarMarcacoesAdmin>>[number];
type Agg = Awaited<ReturnType<typeof agregadoMarcacoes>>;

export type AdminMarcacoesViewProps = {
  agg: Agg | undefined;
  aba: Aba;
  setAba: (a: Aba) => void;
  statusCt: string;
  setStatusCt: (v: string) => void;
  tipoFlag: string;
  setTipoFlag: (v: string) => void;
  statusFlag: string;
...
export function AdminMarcacoesView({
  agg,
  aba,
  setAba,
  statusCt,
    ...
    <div className="mx-auto max-w-7xl px-4 py-10 space-y-8">
      ...
      {aba === "contestacoes" ? (
        <section className="space-y-3">
          ...
          {contestacoes.isLoading ? (
            <div className="text-sm text-muted-foreground flex items-center gap-2">
              <Loader2 className="size-4 animate-spin" /> Carregando…
            </div>
          ) : (contestacoes.data ?? []).length === 0 ? (
        ...
        <section className="space-y-3">
          ...
          ) : (marcacoes.data ?? []).length === 0 ? (
            <p className="text-sm text-muted-foreground">Nenhuma marcação nesses filtros.</p>
          ) : (
            <div className="space-y-3">
              {(marcacoes.data ?? []).map((f) => {
                const href = hrefEntidade(f.entidade_tipo, f.entidade_id);
                return (
                  <InvestigacaoInline
                    key={f.id}
                    ...
                    candidato={{
                      ...
                      severidade: severidadeFromVotos(f.votos_score),
                      detalhes: {
...
function ContestacaoCard({
  ...
  item: Contestacao;
  onSalvar: (status: StatusContestacao, resposta?: string) => Promise<void>;
}) {
  const [status, setStatus] = React.useState<StatusContestacao>(item.status as StatusContestacao);
  const [resposta, setResposta] = React.useState(item.resposta ?? "");
  const [saving, setSaving] = React.useState(false);
  const dirty = contestacaoDirty(item, status, resposta);
