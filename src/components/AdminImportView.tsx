    ...
    <section className="rounded-xl border border-border bg-card/50 p-4 space-y-4">
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-1 max-w-2xl">
          Executa a mesma requisição do import <strong>a partir do servidor</strong> (mesmo IP,
          mesmos headers, mesma chave) e exibe os valores brutos recebidos. A CGU lista por
          vigência, então a consulta é por <strong>página</strong> (sem datas) — útil para conferir
          a página indicada num alerta de qualidade.
        </p>
      </div>
      ...
      {resultado && (
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            ...
            {filtrarId &&
              ` · exibindo apenas id=${filtrarId} (${resultado.contratos.length} encontrado(s))`}
          </p>
          ...
          {resultado.contratos.length === 0 ? (
            <p className="text-xs text-muted-foreground">
              Nenhum contrato encontrado com esse filtro nesta página.
            </p>
          ) : (
    ...
    <section className="rounded-xl border border-border bg-card/50 p-4 space-y-4">
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-1 max-w-2xl">
          ...
          <code>/convenios</code>, <code>/emendas</code>, …) e lista os{" "}
          <strong>nomes de campo reais</strong> — o passo de de-risking antes de escrever/alterar um
          mapper, já que os campos diferem por endpoint.
        </p>
      </div>
      <div className="flex flex-wrap gap-2 items-end">
        <div className="flex flex-col gap-1">
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="text-sm">
              ...
              <div className="text-xs text-muted-foreground mt-0.5">
                {p.batch.done} / {p.batch.total} · {p.batch.importados} importados · {p.batch.erros}{" "}
                erros
              </div>
            </div>
            {p.isRunning && (
              <Button size="sm" variant="ghost" onClick={p.cancelBatch}>
                Cancelar
              </Button>
            )}
          </div>
        ...
        <TabsContent value="portal" className="space-y-4 mt-4">
          ...
          <div className="rounded-xl border border-border bg-card p-5">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Sincroniza os <strong>nomes</strong> dos órgãos a partir do <code>/orgaos-siafi</code>{" "}
              e verifica a <strong>atividade</strong> de cada órgão com dados (execução recente em
              <code>/despesas/por-orgao</code>). Órgãos sem execução recente são marcados como
              <strong> extintos</strong>, mantendo o histórico. Alimenta a lista de{" "}
              <code>/orgaos</code> e o seletor de import abaixo.
            </p>
            <div className="mt-4">
              <Button
                variant="outline"
                onClick={p.onSincronizarCatalogo}
                disabled={p.sincronizandoCatalogo || p.isRunning}
              >
                {p.sincronizandoCatalogo ? (
          ...
          <div className="rounded-xl border border-border bg-card p-5">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Importa contratos do Portal da Transparência (CGU) para o órgão selecionado. Sem
              período, faz uma <strong>varredura completa</strong> do histórico do órgão; cada
              contrato é alocado pelo mês de <strong>início de vigência</strong> (mesma dimensão da
              cobertura). Informe um período de vigência abaixo para importar só parte.
            </p>
            ...
            <p className="text-[11px] text-muted-foreground mt-1">
              Com as duas datas preenchidas, importa só contratos cujo{" "}
              <strong>início de vigência</strong> cai no período (a CGU filtra por vigência). Vazio
              = varredura completa do órgão.
            </p>
            ...
            <p className="text-xs text-muted-foreground mt-2">
              A varredura confere o <strong>detalhe oficial</strong> de cada contrato (corrige o bug
              ÷10.000 da listagem) e roda em <strong>rodadas</strong> de ~3min. Órgãos grandes
              precisam de várias — use “Continuar” abaixo ou ligue o auto-continuar. O Portal
              costuma ter lag de 1–3 meses na publicação de contratos novos.
            </p>
              ...
              <div className="mt-4 rounded-md border border-amber-500/40 bg-amber-500/5 p-3 text-xs space-y-2">
                ...
                <ul className="space-y-1.5">
                  {p.varredurasIncompletas.map((v) => {
                    ...
                    const janela =
                      v.dataInicial && v.dataFinal
                        ? ` · vigência ${v.dataInicial}→${v.dataFinal}`
                        : "";
                    return (
                      <li
                        key={`${v.orgaoCod}${janela}`}
                        className="flex items-center justify-between gap-2"
                      >
                        <span className="text-muted-foreground">
                          {o?.sigla ?? v.orgaoCod} — varrido até a pág. {v.ultimaPagina}
                          {janela}
                        </span>
                        <Button
                          size="sm"
                          ...
                          onClick={() =>
                            p.continuarVarredura(v.orgaoCod, v.dataInicial, v.dataFinal)
                          }
                          disabled={p.isRunning}
                        >
        ...
        <TabsContent value="camara" className="space-y-4 mt-4">
          <div className="rounded-xl border border-border bg-card p-3">
            <PeriodoInline
              ...
              setAno={p.setAno}
              setMes={p.setMes}
              disabled={p.camaraBusy !== null || p.isRunning}
              hint="Mês usado em CEAP e votações; ano usado em proposições."
              years={p.years}
            />
          </div>
          <div className="rounded-xl border border-border bg-card p-5 space-y-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                API <code>dadosabertos.camara.leg.br</code>. Importe primeiro o cadastro, depois
                CEAP por mês (varre ~513 deputados).
              </p>
            </div>
            <div className="flex flex-wrap gap-2 items-end">
              <Button
                ...
                onClick={p.onImportarDeputados}
              >
                {p.camaraBusy === "deputados" ? (
            ...
            <div className="border-t border-border pt-3 space-y-2">
              ...
              <div className="flex flex-wrap gap-2 items-end">
                <div>
                  ...
                  <Input
                    type="number"
                    value={p.legHistIni}
                    onChange={(e) => p.setLegHistIni(Number(e.target.value))}
                    disabled={p.camaraBusy !== null}
                    className="mt-1 w-24"
                  />
                </div>
                <div>
                  ...
                  <Input
                    type="number"
                    value={p.legHistFim}
                    onChange={(e) => p.setLegHistFim(Number(e.target.value))}
                    disabled={p.camaraBusy !== null}
                    className="mt-1 w-24"
                  />
                </div>
                <Button
                  ...
                  size="sm"
                  disabled={p.camaraBusy !== null}
                  onClick={p.onImportarHistCamara}
                >
                  {p.camaraBusy === "hist" ? (
                    <Loader2 className="size-3.5 mr-2 animate-spin" />
                  ) : null}
                ...
                <Button
                  ...
                  size="sm"
                  disabled={p.camaraBusy !== null}
                  onClick={p.onImportarTrajetoriaCamara}
                >
                  {p.camaraBusy === "trajetoria" ? (
                    <Loader2 className="size-3.5 mr-2 animate-spin" />
                  ) : null}
              ...
              <p className="text-xs text-muted-foreground">
                ...
                afastamentos e as vacâncias. É demorado (~1 chamada por deputado); rode depois do
                cadastro.
              </p>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-border bg-card p-5 space-y-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Cada votação + votos individuais. Calcula disciplina partidária. Comece com 15–30
                dias.
              </p>
            </div>
            <div className="flex flex-wrap gap-2 items-end">
              <div>
                ...
                <Input
                  type="date"
                  value={p.votIni}
                  onChange={(e) => p.setVotIni(e.target.value)}
                  disabled={p.camaraBusy !== null}
                  className="mt-1 w-40"
                />
              </div>
              <div>
                ...
                <Input
                  type="date"
                  value={p.votFim}
                  onChange={(e) => p.setVotFim(e.target.value)}
                  disabled={p.camaraBusy !== null}
                  className="mt-1 w-40"
                />
              </div>
        ...
        <TabsContent value="senado" className="space-y-4 mt-4">
          <div className="rounded-xl border border-border bg-card p-3">
            <PeriodoInline
              ...
              setAno={p.setAno}
              setMes={p.setMes}
              disabled={p.senadoBusy !== null || p.isRunning}
              hint="Mês usado em CEAPS e votações; ano usado em matérias."
              years={p.years}
            />
          </div>
          <div className="rounded-xl border border-border bg-card p-5 space-y-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                API <code>legis.senado.leg.br</code>. Cadastro primeiro, depois CEAPS / matérias /
                votações.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              <Button
                variant="outline"
                size="sm"
                disabled={p.senadoBusy !== null}
                onClick={p.onImportarSenadores}
              >
            ...
            <div className="border-t border-border pt-3 space-y-2">
              ...
              <div className="flex flex-wrap gap-2 items-end">
                ...
                <Button
                  ...
                  size="sm"
                  disabled={p.senadoBusy !== null}
                  onClick={p.onImportarHistSenado}
                >
                  {p.senadoBusy === "hist" ? (
                    <Loader2 className="size-3.5 mr-2 animate-spin" />
                  ) : null}
          ...
          {p.history.length === 0 ? (
            ...
            <TooltipProvider delayDuration={150}>
              <div className="overflow-x-auto rounded-xl border border-border">
                    ...
                    <tr>
                      ...
                      <th className="text-right p-3">
                        <Tooltip>
                          ...
                          <TooltipContent className="max-w-xs text-xs">
                            ...
                            consultado (antes de filtros e deduplicação). Disponível apenas para o
                            Portal CGU.
                          </TooltipContent>
                        </Tooltip>
                      </th>
                      ...
                      <th className="text-left p-3">
                        <Tooltip>
                          ...
                          <TooltipContent className="max-w-md text-xs">
                            URL da API externa efetivamente consultada nesta tentativa (método +
                            endpoint + parâmetros). Útil para reproduzir a chamada manualmente via
                            cURL.
                          </TooltipContent>
                        </Tooltip>
                      </th>
                          ...
                          {h.bruto !== null && h.bruto > h.importados ? (
                            <Tooltip>
                              ...
                              <TooltipContent className="max-w-xs text-xs space-y-1">
                                <p>
                                  <strong>
                                    {(h.bruto - h.importados).toLocaleString("pt-BR")}
                                  </strong>{" "}
                                  registro(s) do bruto não foram persistidos.
                                </p>
                                <p className="text-muted-foreground">
                                  Motivos típicos: deduplicação por chave única (mesmo id já
                                  existente), filtros de escopo/órgão, registros descartados por
                                  sanitização LGPD ou campos obrigatórios ausentes, e linhas
                                  rejeitadas por validação de schema.
                                </p>
                              </TooltipContent>
                            </Tooltip>
                          ) : (
                ...
                <HistorySentinel
                  hasMore={p.histHasMore}
                  loading={p.histLoadingMore}
                  onLoadMore={p.loadMoreHistory}
                />
              </div>
            </TooltipProvider>
          )}
        ...
        <TabsContent value="manutencao" className="space-y-4 mt-4">
          <div className="rounded-xl border border-border bg-card p-5 space-y-3">
            ...
            <Button
              variant="outline"
              size="sm"
              disabled={p.sanitizing || p.isRunning}
              onClick={p.onSanitize}
            >
              {p.sanitizing ? (
                <Loader2 className="size-3.5 mr-2 animate-spin" />
              ) : (
          ...
          <Collapsible className="rounded-xl border border-destructive/40 bg-destructive/5">
            ...
            <CollapsibleContent className="px-5 pb-5 space-y-3">
              <p className="text-xs text-muted-foreground">
                Selecione as fontes que quer limpar. Opcionalmente restrinja por intervalo de anos —
                fontes que não têm coluna de período são ignoradas pelo filtro e apagadas por
                completo se selecionadas (cadastros de deputados/senadores, fornecedores, órgãos).
              ...
              <div className="flex flex-wrap gap-2 text-xs">
                <Button
                  size="sm"
                  variant="outline"
                  onClick={p.selecionarTodasFontes}
                  disabled={p.limpBusy}
                >
                  Selecionar todas
                </Button>
              ...
              <AlertDialog>
                <AlertDialogTrigger asChild>
                  ...
                  >
                    ...
                    Apagar {p.limpFontes.size} fonte(s)
                    {p.limpUsarPeriodo ? ` · ${p.limpAnoIni}–${p.limpAnoFim}` : ""}…
                  </Button>
                </AlertDialogTrigger>
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      ...
                      {p.limpUsarPeriodo
                        ? ` no intervalo ${p.limpAnoIni}–${p.limpAnoFim}`
                        : " por completo"}
                  ...
                  />
                  <AlertDialogFooter>
                    <AlertDialogCancel onClick={() => p.setLimpConfirm("")}>
                      Cancelar
                    </AlertDialogCancel>
                    <AlertDialogAction
