  ...
  return (
    <div className="flex flex-col items-center gap-4">
      <svg viewBox={`0 0 ${size} ${size}`} className="w-full max-w-xs">
        ...
        <path d={polyPath} fill="color-mix(in oklab, var(--accent) 25%, transparent)" stroke="var(--accent)" strokeWidth={1.5} />
        {eixos.map((e, i) => {
          const [x, y] = pt(i, 1.18);
          return (
            <text key={i} x={x} y={y} fontSize="10" textAnchor="middle" fill="var(--muted-foreground)" dominantBaseline="middle">
              {e.label}
            </text>
          );
        })}
