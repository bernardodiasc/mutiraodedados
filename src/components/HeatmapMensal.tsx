...
export function HeatmapMensal({ contratos }: { contratos: Contrato[] }) {
  ...
  if (anos.length === 0 || max === 0) {
    return (
      <div className="text-sm text-muted-foreground">Sem dados suficientes para o heatmap.</div>
    );
  }
    ...
    <div className="overflow-x-auto">
      <div className="inline-block min-w-full">
        <div className="grid" style={{ gridTemplateColumns: `48px repeat(12, minmax(28px, 1fr))` }}>
          <div />
          {MESES.map((m) => (
            <div key={m} className="text-[10px] uppercase text-muted-foreground text-center py-1">
              {m}
            </div>
          ))}
          {anos.map((ano) => (
          ...
          <div
            ...
            style={{
              background:
                v === 0
                  ? "transparent"
                  : `color-mix(in oklab, var(--accent) ${o * 100}%, transparent)`,
            }}
          />
        );
      })}
