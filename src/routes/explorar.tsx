  ...
  head: () => ({
    meta: [
      { title: "Explorar por ente — Mutirão de Dados" },
      {
        name: "description",
        content:
          "Visão consolidada de um estado ou município: contratos, relatórios fiscais e transferências da União em um só lugar.",
      },
...
function ExplorarPage() {
  ...
  const siconfi = useQuery({
    ...
    enabled: !!enteSelecionado,
    queryFn: () =>
      siconfiFn({ data: { uf: uf || undefined, codIbge: ibge || undefined, limit: 100 } }),
  });
    ...
    <div className="mx-auto max-w-7xl px-4 py-10 space-y-8">
      <header>
        ...
        <p className="text-muted-foreground mt-3 max-w-3xl leading-relaxed">
          Escolha um ente federativo para ver, no mesmo lugar, dados das três fontes nacionais:
          contratações (PNCP), relatórios fiscais (SICONFI) e transferências da União
          (Transferegov/CGU).
        </p>
      </header>
        ...
        <>
          <section className="grid gap-4 sm:grid-cols-4">
            <Stat
              ...
              value={String(totais.contratos)}
            />
            <Stat
              icon={<Building2 className="size-4" />}
              label="Total contratado"
              value={fmtBRL(totais.contratado)}
            />
            <Stat
              icon={<ArrowRightLeft className="size-4" />}
              label="Transferências recebidas"
              value={fmtBRL(totais.transferidoTotal)}
            />
            <Stat
              icon={<Landmark className="size-4" />}
              label="Linhas SICONFI"
              value={String(totais.relatorios)}
            />
          </section>
          ...
          {aba === "contratos" && (
            <section className="space-y-3">
              <ExportRow
                ...
                onClick={() => downloadCSV(`pncp_${uf || ibge}`, pncp.data?.contratos ?? [])}
              />
              ...
              {!pncp.isLoading && (pncp.data?.contratos.length ?? 0) === 0 && (
                <p className="text-sm text-muted-foreground">
                  Nenhum contrato PNCP em cache para este ente.
                </p>
              )}
          ...
          {aba === "fiscal" && (
            <section className="space-y-3">
              <ExportRow
                ...
                onClick={() => downloadCSV(`siconfi_${uf || ibge}`, siconfi.data?.relatorios ?? [])}
              />
