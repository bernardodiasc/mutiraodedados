...
export const Route = createFileRoute("/transferencias/especiais/$id")({
  component: EmendaDetalhe,
  head: () => ({ meta: [{ title: "Transferência Especial — Auditoria Cidadã" }] }),
});

function EmendaDetalhe() {
  const { id } = useParams({ from: "/transferencias/especiais/$id" });
  ...
  return (
    <article className="mx-auto max-w-3xl px-4 py-10 space-y-6">
      ...
      <header>
        <div className="text-xs uppercase tracking-wider text-muted-foreground">Transferência Especial (EC 105)</div>
      ...
      <section className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-4 text-sm space-y-2">
        <div className="flex items-center gap-2 font-semibold text-amber-700 dark:text-amber-300">
          <Info className="size-4" /> Guia do auditor cidadão
        </div>
        <p className="text-muted-foreground">
          Em Transferências Especiais (EC 105/2019), o valor é repassado diretamente ao
          ente beneficiário e passa a integrar o caixa local — por isso o
          <strong className="text-foreground"> "valor pago" no Transferegov aparece como R$ 0</strong>,
          mesmo quando o recurso já foi disponibilizado. A execução é feita por estados e
          municípios, fora do SIAFI federal.
        </p>
        <p className="text-muted-foreground">
          Para auditar o uso real, consulte:
        </p>
        <ul className="list-disc pl-5 text-muted-foreground space-y-1">
          <li>Portal de transparência do {e.municipio_nome ?? "ente beneficiário"} {e.uf ? `(${e.uf})` : ""} — empenhos, liquidações e pagamentos.</li>
          <li>SICONFI (Tesouro Nacional) — RREO/RGF do ente.</li>
          <li>PNCP — contratos e licitações custeados com o recurso.</li>
          <li>Tribunal de Contas estadual/municipal — relatórios e julgamentos.</li>
        </ul>
        <p className="text-xs text-muted-foreground/80">
          Diferenças entre o valor disponibilizado aqui e o executado localmente são esperadas e
          devem ser investigadas caso a caso.
        </p>
      </section>

      <a
        href={`https://portaldatransparencia.gov.br/emendas/consulta?de=${e.ano}&ate=${e.ano}&codigoEmenda=${encodeURIComponent(e.codigo_emenda ?? e.numero_emenda ?? "")}`}
        target="_blank"
