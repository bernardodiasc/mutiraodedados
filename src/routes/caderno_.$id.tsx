...
import { listarItensDaPergunta, removerItem, adicionarItem } from "@/lib/pergunta-itens.functions";
...
function CadernoDetalhePage() {
  ...
  const acaoEncerrar = useMutation({
    ...
    onSuccess: () => {
      toast.success("Investigação encerrada");
      invalidate();
    },
    onError: (e) => toast.error(e instanceof Error ? e.message : "Erro"),
  });
  ...
  if (loading || isLoading) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-16 text-muted-foreground flex items-center gap-2">
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12">
      ...
      <div className="mt-2 text-[11px] text-muted-foreground">
        Criada em {formatarDataPt(pergunta.created_at)} · Atualizada em{" "}
        {formatarDataPt(pergunta.updated_at)}
      </div>
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        {podeSolicitar && (
          <Button
            size="sm"
            onClick={() => acaoSolicitar.mutate()}
            disabled={acaoSolicitar.isPending}
          >
        ...
        {podeEncerrar && (
          <Button
            size="sm"
            variant="outline"
            onClick={() => acaoEncerrar.mutate()}
            disabled={acaoEncerrar.isPending}
          >
        ...
        {podeArquivar && (
          <Button
            size="sm"
            variant="outline"
            onClick={() => acaoArquivar.mutate()}
            disabled={acaoArquivar.isPending}
          >
        ...
        {podeReabrir && (
          <Button
            size="sm"
            variant="outline"
            onClick={() => acaoReabrir.mutate()}
            disabled={acaoReabrir.isPending}
          >
        ...
        <Button
          ...
          variant="ghost"
          onClick={() => {
            if (confirm("Excluir esta pergunta e tudo dentro dela?")) acaoExcluir.mutate();
          }}
      ...
      <section className="mt-10">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <Button
            size="sm"
            variant="outline"
            onClick={() => setFormAberto(formAberto === "link" ? null : "link")}
          >
          ...
          <Button
            size="sm"
            variant="outline"
            onClick={() => setFormAberto(formAberto === "anotacao" ? null : "anotacao")}
          >
            <StickyNote className="size-3.5 mr-1" /> Adicionar nota
          </Button>
          <span className="text-xs text-muted-foreground self-center">
            Para salvar contratos, fornecedores, parlamentares etc., use{" "}
            <strong>Salvar no caderno</strong> na página do item.
          </span>
        </div>
        {formAberto && (
          <div className="mt-3 border border-border rounded-xl p-4 bg-card space-y-3">
            ...
            <Textarea
              placeholder={
                formAberto === "link" ? "Anotação opcional sobre este link" : "Escreva a nota"
              }
              value={fNota}
            ...
            <div className="flex gap-2 justify-end">
              ...
              <Button
                ...
                disabled={
                  acaoAdicionar.isPending || (formAberto === "link" ? !fUrl.trim() : !fNota.trim())
                }
              >
          ...
          <>
            ...
            <ul className="mt-3 grid gap-2">
              {itens.map((it) => (
                <li
                  key={it.id}
                  className="border border-border rounded-xl p-4 bg-card flex items-center gap-3"
                >
                  <input
                    type="checkbox"
