...
export const Route = createFileRoute("/orgaos_/$cod")({
  ...
  notFoundComponent: () => (
    <div className="mx-auto max-w-3xl px-4 py-20 text-center">
      ...
      <Link to="/orgaos" className="text-accent mt-4 inline-block">
        Voltar à lista
      </Link>
    </div>
  ),
...
function OrgaoDetail() {
  ...
  const novosUltimoAno = [...firstSeen.values()].filter(
    (d) => new Date(d).getFullYear() === ultimoAno,
  ).length;
  ...
  const eixos: RadarEixo[] = [
    {
      label: "Concentração",
      valor: concentracao,
      descricao: `Top fornecedor = ${(concentracao * 100).toFixed(0)}% do contratado.`,
    },
    {
      label: "% Dispensa",
      valor: pctDispensa,
    ...
    },
    {
      label: "Crescimento",
      valor: crescimento,
      descricao: `Variação relativa do gasto vs ano anterior.`,
    },
    {
      label: "Forn. novos",
      valor: novosScore,
      descricao: `${novosUltimoAno} novos fornecedores em ${ultimoAno || "—"}.`,
    },
    {
      label: "Ticket médio",
      valor: ticketScore,
    ...
    },
    {
      label: "Fragmentação",
      valor: fragScore,
      descricao: `${fragCount} dispensas abaixo de R$ 17.600.`,
    },
  ];
      ...
      {orgao && (
        <>
          ...
          <div className="mt-4 border border-dashed border-border rounded-xl p-4 bg-card text-sm text-muted-foreground">
            <strong className="text-foreground">Contexto:</strong> os {fmtBRL(total)} contratados
            representam {descreverValor(total)}.
            {medFuncao > 0 && (
              <>
                {" "}
                Para órgãos da função <em>{base.funcao}</em>, a mediana contratada nesta amostra é{" "}
                {fmtBRL(medFuncao)} — este órgão está em{" "}
                <strong className="text-foreground">
                  {total >= medFuncao
                    ? `${(total / medFuncao).toFixed(1)}× a mediana`
                    : `${((total / medFuncao) * 100).toFixed(0)}% da mediana`}
                </strong>
                .
              </>
            )}{" "}
            <MetodologiaPopover titulo="Como o contexto é calculado">
              <p>
                Mediana = ponto central dos totais contratados por órgãos da mesma função,
                considerando apenas os dados já carregados na plataforma.
              </p>
              <p>
                Comparação com salário mínimo usa o valor nacional vigente para 2026, para fins
                didáticos.
              </p>
            </MetodologiaPopover>
          </div>
            ...
            <div className="mt-4 border border-border rounded-xl bg-card p-5">
              <div className="flex items-baseline justify-between flex-wrap gap-3">
                <div>
                  <div className="text-xs uppercase tracking-wider text-muted-foreground">
                    Índice de Transparência Institucional
                  </div>
                  <div className="mt-1 flex items-baseline gap-3">
                    <span className={`font-display text-3xl ${corDaFaixa(iti.faixa)}`}>
                      {iti.nota}
                      <span className="text-sm text-muted-foreground font-mono">/100</span>
                    </span>
                    <span className={`text-xs uppercase tracking-wider ${corDaFaixa(iti.faixa)}`}>
                      {rotuloDaFaixa(iti.faixa)}
                    </span>
                  </div>
                </div>
                <Link
              ...
              <div className="mt-4 grid sm:grid-cols-5 gap-2">
                {iti.componentes.map((c) => (
                  <div
                    key={c.chave}
                    className="border border-border rounded-lg p-2 bg-background/40"
                  >
              ...
              <p className="mt-3 text-xs text-muted-foreground">
                Mede a <strong className="text-foreground">clareza informacional</strong> do que o
                órgão publica. Não avalia legalidade nem eficiência.
              </p>
            </div>
          )}
          ...
          <div className="mt-10">
            <div className="flex items-baseline justify-between mb-3">
              ...
              <MetodologiaPopover titulo="Série histórica">
                <p>
                  Soma anual dos valores dos contratos assinados pelo órgão, sem deflação. Mostra o
                  nominal — o reajuste pelo IPCA virá em fase posterior.
                </p>
              </MetodologiaPopover>
            </div>
            ...
            <div>
              <div className="flex items-baseline justify-between mb-3">
                ...
                <MetodologiaPopover titulo="Eixos do radar">
                  <p>
                    Cada eixo é normalizado de 0 a 1 a partir dos contratos carregados. Quanto mais
                    próximo da borda, mais atenção o eixo merece — sempre como sinal, nunca como
                    prova.
                  </p>
                </MetodologiaPopover>
              </div>
            ...
            <div>
              <div className="flex items-baseline justify-between mb-3">
                ...
                <MetodologiaPopover titulo="Heatmap mensal">
                  <p>
                    Cada célula = soma dos contratos assinados naquele mês. Padrões anormais (ex.:
                    pico de dezembro) podem indicar contratos de fim de ano para empenhar saldo.
                  </p>
                </MetodologiaPopover>
              </div>
              <div className="border border-border rounded-xl p-4 bg-card">
                  ...
                  {contratos.slice(0, 15).map((c) => (
                    <tr key={c.id} className="hover:bg-muted/50">
                      <td className="p-3">
                        ...
                        >
                          {sanitizarTextoPublico(c.objeto)}
                        </Link>
                      </td>
