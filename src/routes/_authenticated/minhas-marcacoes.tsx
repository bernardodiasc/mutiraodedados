...
import { statusMarcacoesUsuario, type MarcacaoStatus } from "@/lib/data/marcacoes.functions";
      ...
      <div className="mt-8 space-y-3">
        {loading ? (
          ...
          <p className="text-sm text-muted-foreground">
            Você ainda não marcou nada. Abra uma ficha de órgão, fornecedor ou contrato e use o
            painel "Marcações cidadãs".
          </p>
        ) : (
          flags.map((f) => {
            const href =
            ...
            const st: MarcacaoStatus | undefined = (statusMap as Record<string, MarcacaoStatus>)[
              key
            ];
            ...
            return (
              <Link
                key={f.id}
                to={href}
                className="block border border-border rounded-xl p-4 bg-card hover:border-accent"
              >
                <div className="flex items-center justify-between gap-2 flex-wrap">
                  ...
                  {label && (
                    <span
                      className={`text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full ${label.cls}`}
                    >
                      {label.txt}
                    </span>
                  )}
