ALTER TABLE public.itens_salvos
  ADD COLUMN conteudo_snapshot text
    CHECK (conteudo_snapshot IS NULL OR length(conteudo_snapshot) <= 20000),
  ADD COLUMN snapshot_em timestamptz,
  ADD COLUMN snapshot_hash text,
  ADD COLUMN snapshot_verificado_em timestamptz,
  ADD COLUMN snapshot_divergiu_em timestamptz;
