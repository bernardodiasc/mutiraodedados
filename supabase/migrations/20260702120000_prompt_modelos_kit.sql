...
-- N:N aos mapas investigativos (artigos com categoria 'mapa'). O prompt é um
-- objetivo que reaproveita os passos do mapa; um prompt genérico serve a vários mapas.

-- 1. prompt_modelos (curados por admin)
...
-- 2. mapa_prompts (associação N:N artigo ↔ prompt)
...
-- 3. RLS — público lê prompts ativos vinculados a mapa público; CRUD só admin.
...
-- 4. Prompts podem entrar em pastas do caderno (pergunta_itens).
ALTER TYPE public.pergunta_item_tipo ADD VALUE IF NOT EXISTS 'prompt';

-- 5. Piloto CEAP/CEAPS: 4 prompts-semente vinculados ao mapa
--    'auditar-cota-parlamentar'. Todos carregam a ressalva de que reembolso
--    aprovado não é irregularidade — sinal ≠ prova.
WITH mapa AS (
  ...
  (
    ...
    'Você vai analisar despesas de cota parlamentar (CEAP/CEAPS) de {{parlamentar}} no período {{periodo}}. Os dados abaixo vieram do CSV exportado da página do parlamentar (fonte: Câmara/Senado).
...
{{cole_o_csv}}',
  ...
  (
    ...
    'Você vai analisar a série mensal de despesas de cota parlamentar de {{parlamentar}} em {{ano}}. O teto mensal da cota é {{teto_mensal}}.
...
{{cole_o_csv}}',
  ...
  (
    ...
    'Você vai comparar as despesas de cota parlamentar de {{parlamentar}} com as de outros parlamentares do mesmo {{recorte}} (partido ou estado).
...
{{cole_csv_B}}',
  ...
  (
    ...
    'Você vai fazer o raio-x de um fornecedor que apareceu com destaque nas despesas de cota parlamentar. Abaixo estão os dados cadastrais e de recebimentos que colhi na página do fornecedor (CNPJ) e em fontes oficiais.
